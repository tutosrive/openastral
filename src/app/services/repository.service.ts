...
class RepositoryService extends Service {
    ...
    async getSearch(text: string, page: number, type: 'repository' | 'mix'): Promise<any[] | null> {
        let result: Tables<'repository'>[] | null = null;
        this.start = (page - 1) * this.PAGE_COUNT;
        const FUNC_NAME = type === 'repository' ? 'find_repositories' : 'find_repositories_mix';
        const { data, error } = await this.client.rpc(FUNC_NAME, { regextofind: text, startl: this.start, total: this.PAGE_COUNT });
        if (!error) {
